...
if [ ! -f dwi.mif.gz ]; then
    ...
    dwiextract -shells 1000,2600 dwiAP.mif.gz tmp_dwiAP_b1000b2600.mif
...
if [ ! -f dwi_den.mif.gz ]; then
    ...
    # PCA-denoising
    dwidenoise dwi.mif.gz dwi_den.mif.gz -noise denoise/dwi_noise.mif.gz -nthreads 4;
...
if [ ! -f dwi_den_unr.mif.gz ]; then
    ...
    # Gibbs 
    mrdegibbs -axes 0,1 dwi_den.mif.gz dwi_den_unr.mif.gz -nthreads 4
...
if [ -f topup/b0APPA.mif.gz ]; then # We have b0APPA
